...
def scr(stock_df, df, index_name):
    n = -1
    stocklist = df['Symbol'].to_list()[1:]
    name = df['Company Name'].to_list()[1:]
    ...
    rs_df = pd.DataFrame(list(zip(stocklist, returns_multiples)), columns=[
                         'Ticker', 'Returns_multiple'])
    ...
    for stock, na in zip(stocklist, name):
        # print(n)
        try:
            stock = stock + '.NS'
            n += 1
...
def screening(df, index_name):
    yf.pdr_override()

    # Variables
    tickers = df['Symbol'].to_list()[1:]
    name = df['Company Name'].to_list()[1:]
    tickers = [item + '.NS' for item in tickers]
    dct = {}
    ...
    exportList = exportList.reset_index(drop=True)
    print('\n', exportList)
    return exportList
